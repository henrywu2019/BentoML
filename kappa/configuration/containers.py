...
class KappaContainer(containers.DeclarativeContainer):
    ...
    prometheus_multiproc_dir = providers.Callable(
        os.path.join, kappa_home, "prometheus_multiproc_dir",
    )

    bento_bundle_deployment_version = providers.Callable(
        get_kappa_deploy_version,
        providers.Callable(
    ...
    gamma_database_url = providers.Callable(
        ...
        providers.Callable(
            "sqlite:///{}".format,
            providers.Callable(os.path.join, kappa_home, "storage.db"),
        ),
    ...
    gamma_file_system_directory = providers.Callable(
        lambda default, customized: customized or default,
        providers.Callable(os.path.join, kappa_home, "repository"),
    ...
    logging_file_directory = providers.Callable(
        lambda default, customized: customized or default,
        providers.Callable(os.path.join, kappa_home, "logs",),
