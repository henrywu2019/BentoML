...
from kappa.gamma.utils import ensure_node_available_or_raise, parse_grpc_url
...
def get_gamma_service(
    ...
    if channel_address:
        ...
        import grpc
        from kappa.gamma.client.interceptor import header_client_interceptor
        from kappa.gamma.proto.gamma_service_pb2_grpc import GammaStub
        ...
        from kappa.gamma.repository import create_repository
        from kappa.gamma.gamma_service_impl import get_gamma_service_impl
...
def start_gamma_service_grpc_server(
    ...
    from kappa.gamma.repository import create_repository
    from kappa.gamma.gamma_service_impl import get_gamma_service_impl
    from kappa.gamma.proto.gamma_service_pb2_grpc import add_GammaServicer_to_server
    from kappa.gamma.proto.gamma_service_pb2_grpc import GammaServicer
    ...
    if debug_mode:
        try:
            ...
            from kappa.gamma.proto import gamma_service_pb2
    ...
    click.echo(
        ...
        f"https://confluence.oci.oraclecorp.com/display/OCAS/ML+Engineering\n"
        f'{f"* Web server log can be found here: {web_ui_log_path}" if with_ui else ""}'
        f"\n-----\n"
        f"* Usage in Python:\n"
        f'*  my_model.save(gamma_url="0.0.0.0:{grpc_port}")\n'
        f"*  from kappa.gamma.client import get_gamma_client\n"
        f'*  get_gamma_client("0.0.0.0:{grpc_port}").repository.list()\n'
        f"* Usage in CLI:\n"
        f"*  kappa list --gamma-url=0.0.0.0:{grpc_port}\n"
        f"*  kappa containerize IrisClassifier:latest --gamma-url=0.0.0.0:"
        f"{grpc_port}\n"
        f"*  kappa push IrisClassifier:20200918001645_CD2886 --gamma-url=0.0.0.0:"
        f"{grpc_port}\n"
        f"*  kappa pull IrisClassifier:20200918001645_CD2886 --gamma-url=0.0.0.0:"
        f"{grpc_port}\n"
        f"*  kappa retrieve IrisClassifier:20200918001645_CD2886 "
        f'--gamma-url=0.0.0.0:{grpc_port} --target_dir="/tmp/foo/bar"\n'
        f"*  kappa delete IrisClassifier:20200918001645_CD2886 "
