...
const ALL_NAMESPACE_TAG = "__KAPPA_ALL_NAMESPACE";
...
const createAPIRoutes = (app, gammaClient) => {
  ...
  router.get("/api/ListProject", async (req: Request, res: Response) => {
    try {
      const result = await gammaClient.listProject();
      logger.info({
        request: "ListProject",
        result: result,
      });
      ...
      logger.error({ request: "ListProject", error: JSON.stringify(error) });
      return res.status(500).json(error);
    }
  });
