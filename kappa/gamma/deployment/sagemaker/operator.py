...
from kappa.gamma.deployment.utils import (
...
from kappa.gamma.proto.repository_pb2 import GetBentoRequest, BentoUri
...
RUN if [ -f /bento/kappa-init.sh ]; then bash -c /bento/kappa-init.sh; fi
...
def _aws_client_error_to_kappa_exception(e, message_prefix=None):
...
def _delete_sagemaker_model_if_exist(sagemaker_client, sagemaker_model_name):
    try:
        ...
        raise generate_kappa_exception_from_aws_client_error(
    ...
    sagemaker_model_info = {
        ...
        "PrimaryContainer": {
            ...
            "Environment": {
                ...
                "KAPPA_GUNICORN_TIMEOUT": str(spec.timeout),
            },
        },
    ...
    def describe(self, deployment_pb):
        try:
            ...
            try:
                ...
                raise _aws_client_error_to_kappa_exception(
                    e,
