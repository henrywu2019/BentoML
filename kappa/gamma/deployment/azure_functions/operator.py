...
from kappa.gamma.deployment.azure_functions.constants import (
...
from kappa.gamma.deployment.azure_functions.templates import AZURE_API_FUNCTION_JSON
...
from kappa.gamma.proto.repository_pb2 import GetBentoRequest
...
def _init_azure_functions_project(
    ...
    try:
        local_kappa_path = os.path.dirname(__file__)
        shutil.copytree(bento_path, azure_functions_project_dir)
        shutil.copy(
            os.path.join(local_kappa_path, 'host.json'),
            os.path.join(azure_functions_project_dir, 'host.json'),
        )
        shutil.copy(
            os.path.join(local_kappa_path, 'local.settings.json'),
            os.path.join(azure_functions_project_dir, 'local.settings.json'),
        )
        shutil.copy(
            os.path.join(local_kappa_path, 'Dockerfile'),
        ...
        os.mkdir(app_path)
        shutil.copy(
            os.path.join(local_kappa_path, 'app_init.py'),
