...
from kappa.cli.deployment import _print_deployment_info, _print_deployments_info
...
from kappa.gamma.deployment import ALL_NAMESPACE_TAG
from kappa.exceptions import CLIException
from kappa.utils import status_pb_to_error_code_and_message
...
def get_azure_functions_sub_command():
    ...
    @click.option(
        ...
        '--namespace',
        type=click.STRING,
        help='Deployment namespace managed by Kappa, the default value is "dev" '
        'which can be changed in Kappa configuration file',
    )
    @click.option(
        '--min-instances',
        type=click.INT,
