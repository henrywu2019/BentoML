...
if [[ "$(docker images -q iad.ocir.io/axhheqi2ofpb/kappa/gamma-service:dev 2> /dev/null)" != "" ]]; then
  ...
  script."
  echo "To rebuild the gamma server dev image, run 'docker rmi iad.ocir.io/axhheqi2ofpb/kappa/gamma-service:dev' and run this script again."
  exit 0
fi

docker build -t iad.ocir.io/axhheqi2ofpb/kappa/gamma-service:dev -f- $GIT_ROOT <<EOF
FROM iad.ocir.io/axhheqi2ofpb/kappa/gamma-service:latest
